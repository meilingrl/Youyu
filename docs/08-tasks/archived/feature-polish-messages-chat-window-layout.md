...
- Fixed the messages workspace with viewport-bounded layout height, `min-height: 0` flex containment, and internal scrolling on both the conversation list and active thread.
- Verified in the browser that `/app/messages` keeps the composer reachable and the thread scrolls inside the panel instead of stretching the full route.
