...
- Fixed the current refund path from live backend behavior instead of a UI-only workaround, then aligned the buyer/admin trade flows with the repaired state transitions.
- Added logistics detail rendering and an explicit map fallback for missing provider configuration so order detail remains honest when no live map integration is available.
