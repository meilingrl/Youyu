...
- [x] Core user purchase path has automated regression coverage
- [x] Admin governance path has automated regression coverage
- [x] Backend tests cover key failure paths, not only happy paths
- [x] Frontend or E2E coverage exists for major visible flows with current user impact
- [x] New tests use shared helpers where duplication would otherwise grow quickly
...
Added 41 new backend test methods across 5 new test classes, 5 new frontend store tests, and 23 new E2E tests across 3 new spec files. Created a shared `BackendTestBase` with common flow helpers to reduce duplication.

### Backend (58 → 78 tests, all passing)
- `BackendTestBase.java` — shared base class with `MockMvc`, `JdbcTemplate`, and 10+ helper methods
- `AdminGovernanceTest.java` — 21 tests: dashboard, user CRUD, verification review, product/shop management, search governance, auth guards
- `CartEdgeCaseTest.java` — 6 tests: quantity merge, update, remove, not-found, empty cart, auth
- `PaymentEdgeCaseTest.java` — 6 tests: non-existent order, cross-user, double initiation, bogus payment, gateway, auth
- `UserProfileTest.java` — 8 tests: profile, verification, addresses CRUD, default address, logout, auth/me

### Frontend (6 → 7 test files, 30 tests, all passing)
- `app.test.js` — 5 tests: keyword, hasKeyword, collapsed toggle

### E2E (1 → 4 spec files, 21 → 42 tests)
- `admin-governance.spec.js` — 18 tests covering admin dashboard, users, verifications, shops, products, search governance, authorization
- `offline-order.spec.js` — full offline order + refund flow
...
- E2E tests depend on a fresh backend with seed data for clean isolation
- Admin governance E2E tests that modify state (user status, verification review, shop status) may conflict with concurrent test runs
