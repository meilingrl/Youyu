...
- `original-root/AGENTS.outer-repo.md` — AGENTS file from the old outer repo where `Youyu/` was a subdirectory. Paths like `Youyu/backend` no longer apply.
- `original-root/.github/workflows/ci.outer-repo.yml` — CI workflow from the old repo. Uses `working-directory: Youyu/backend` which is invalid in the current layout.
